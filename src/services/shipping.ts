...
export class ShippingService {
  ...
  async createRule(brandId: string, data: CreateShippingRuleInput) {
    const rule = this.shippingRepo.create({ brandId, ...data });
  ...
  async updateRule(
    ...
    const rule = await this.shippingRepo.findOne({ where: { id, brandId } });

    if (!rule) {
      throw new AppError(404, "Shipping rule not found");
    }
